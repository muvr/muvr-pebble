...
TEST_F(fixed_queue_test, simple) {
    ...
    uint8_t buffer[1];
    EXPECT_EQ(1, queue_pop(q, buffer, 1)); EXPECT_EQ(1, buffer[0]);
    EXPECT_EQ(1, queue_pop(q, buffer, 1)); EXPECT_EQ(2, buffer[0]);
    EXPECT_EQ(1, queue_pop(q, buffer, 1)); EXPECT_EQ(3, buffer[0]);
    EXPECT_EQ(0, queue_pop(q, buffer, 1));
...
TEST_F(fixed_queue_test, edge) {
    ...
    // cannot pop into nullptr buffer
    EXPECT_EQ(0, queue_pop(q, nullptr, 0));
    // cannot pop into too small buffer
    uint8_t buffer[1];
    EXPECT_EQ(0, queue_pop(q, buffer, 0));
