...
TEST_F(fixed_queue_test, simple) {
    ...
    uint8_t buffer[1];
    uint32_t key;
    EXPECT_EQ(1, queue_peek(q, &key, buffer, 1)); EXPECT_EQ(1, buffer[0]); EXPECT_EQ(0xface0fb0, key); queue_tail(q);
    EXPECT_EQ(1, queue_peek(q, &key, buffer, 1)); EXPECT_EQ(2, buffer[0]); EXPECT_EQ(0xface0fb0, key); queue_tail(q);
    EXPECT_EQ(1, queue_peek(q, &key, buffer, 1)); EXPECT_EQ(3, buffer[0]); EXPECT_EQ(0xface0fb0, key); queue_tail(q);
    EXPECT_EQ(0, queue_peek(q, &key, buffer, 1));
...
TEST_F(fixed_queue_test, edge) {
    ...
    // OK to add one
    EXPECT_EQ(1, queue_add(q, 0xface0fb0, make_buffer(1), 1));

    // cannot peek into nullptr buffer
    EXPECT_EQ(1, queue_peek(q, nullptr, nullptr, 0));
    // cannot peek into too small buffer
    uint8_t buffer[1];
    uint32_t key;
    EXPECT_EQ(1, queue_peek(q, &key, buffer, 0));
